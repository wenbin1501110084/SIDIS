...
std::vector<double> YValues2, pTValues2, F1qgV, F2qgV, F1ggV, F3ggV, F6ggV, FadjV;
...
const double Y1max = 3.;
const double Y1min = -1.0;
const double Y2max = 3.;
const double Y2min = -1.0;
const double Q2minIn = 1.0;// GeV^2
const double Q2maxIn = 2.0;// GeV^2
const double Rcut = 0.4;
const int num_threads = 10;
const long long int sample_points = 10;
...
int zh_kp2_b_integrated(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    double z1 = x[0] * 0.1 + 0.25;
    double z2 = x[1] * 0.1 + 0.25;
    ...
    double Q2 = x[9] * (helper->Q2max - helper->Q2min) + helper->Q2min;
    double kT1 = p1mag/z1;
    double kT2 = p2mag/z2;
    ...
    if (y_inelasticity >= 0.75 || y_inelasticity <= 0.65) { // experimental cut
        f[0] = 0.0;
        f[1] = 0.0;
        return 0;
    }
    ...
    double total_volume = (helper->y1max - helper->y1min) * (helper->y2max - helper->y2min) * (helper->p1magmax - helper->p1magmin) *
                          (pt2maxtep - helper->p2magmin) *  2. * M_PI* 2. * M_PI *  helper->kpmagmax * 0.1*0.1;
...
int Calculate_trigger(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    PARAMETERS *helper = (PARAMETERS*)userdata;
    double z1max = 0.35; double z1min = 0.25;
    double y_inelasticitymax = 0.75; double y_inelasticitymin = 0.65;
    double y_inelasticity = x[0] * (y_inelasticitymax - y_inelasticitymin) + y_inelasticitymin;
    double Q2 = x[1] * (helper->Q2max - helper->Q2min) + helper->Q2min;
    double Q = sqrt(Q2);
    double p1mag = x[2] * (helper->p1magmax - helper->p1magmin) + helper->p1magmin;
    double thetap =  x[3] * 2.* M_PI;
    double eta1 = x[4] * (helper->y1max - helper->y1min) + helper->y1min;
    double zh1 = x[5] * (z1max - z1min) + z1min;
    double lmag = x[6] * 100.;
    double thetal =  x[7] * 2.* M_PI;
    double total_volume = (y_inelasticitymax - y_inelasticitymin) * (helper->p1magmax - helper->p1magmin) *
                          (helper->y1max - helper->y1min) *  (helper->Q2max - helper->Q2min) * 2. * M_PI* 2. * M_PI * 
                          (z1max - z1min) * 100.;
                          
    double kT1 = p1mag/zh1;
    double kTx1 = kT1 * cos(thetap); 
    double kTy1 = kT1 * sin(thetap); 
    double mub2 = kT1 * kT1;
    double zm1 = kT1 * exp(eta1)/sqrt(y_inelasticity)/rootsnn;
    double Qbar2 = zm1 * (1.-zm1) * Q2;
    double xA = Q2/y_inelasticity/rootsnn/rootsnn + kT1 * exp(-eta1)/sqrt(y_inelasticity)/rootsnn;
    if (zm1 > 1. || xA > 1.) {
        f[0] = 0.0;
        return 0;
    }
    double kmlx = kTx1 - lmag*cos(thetal);
    double kmly = kTy1 - lmag*sin(thetal);
    double kmlmag2 = pow(kmlx, 2.) + pow(kmly, 2.);  
    double Hard = 3.*alpha_em *alpha_em /M_PI /Q2 / y_inelasticity * ( 
                  (1.-y_inelasticity)*8.*zm1*zm1*zm1 * (1.-zm1) * (1.-zm1) * 
                  pow(Q/(kT1*kT1 + Qbar2) - Q/(kmlmag2 + Qbar2), 2.) + 
                  (1.+(1.-y_inelasticity)*(1.-y_inelasticity)) * zm1 * (zm1*zm1 + (1.-zm1) *(1.-zm1)) * 
                  (pow(kTx1/(kT1*kT1 + Qbar2) - kmlx/(kmlmag2 + Qbar2), 2.) + pow(kTy1/(kT1*kT1 + Qbar2) - kmly/(kmlmag2 + Qbar2), 2.))
                  );
    
    double rapidity = log(0.01/xA);
    if (rapidity< 0.0) rapidity = 0.0;
    int y_index = int(rapidity/Y_step);
    if (kT1 > maxpT) kT1 = maxpT;
    if (kT1 < minpT) kT1 = minpT;
    std::vector<double> xValues, F1qgVt;
    xValues.clear(); F1qgVt.clear(); 
    for (int inn= y_index * Klength; inn < y_index * Klength + Klength; inn++) {
        xValues.push_back(pTValues2[inn]);
    ...
    inter_F1qgVt = (inter_F1qgVt2 * (rapidity - YValues2[y_index*Klength-Klength] ) + 
                    inter_F1qgVt * (YValues2[y_index*Klength] - rapidity) ) / Y_step;
    if (xA>0.01) {
        double ratio = pow(1.-xA, 4.) / 0.96059601; // 0.96059601 = (1-0.01)^4
        inter_F1qgVt =  ratio * inter_F1qgVt;
    }
    int is=1, ih=1, ic=0, io=1; // ih =1 :pion
    double u1, ub1, d1, db1, s1, sb1, c1, bb1, gl1;
    fdss_(&is, &ih, &ic, &io, &zh1, &mub2, &u1, &ub1, &d1, &db1, &s1, &sb1, &c1, &bb1, &gl1);
    double Dqxqx1 = 4./9. * u1/zh1 + 1./9. * d1/zh1 + 1./9. * s1/zh1;
    double prefactor = p1mag /zh1/zh1 * lmag/4./M_PI/M_PI;
    inter_F1qgVt = 2.*M_PI*M_PI * inter_F1qgVt / 3. / kT1 / kT1;
    
    double dsigmaqgqg_dDeltaphi = prefactor  * Dqxqx1 * inter_F1qgVt * Hard; // inter_F1qgVt is the F 
...
int main(int argc, char* argv[]) 
{
    ...
    inputFile.close();
        ...
        // Open the input file
    std::ifstream inputFile2("Paul_table/Regularged_FT_proton_MV_Paul");
    if (!inputFile2.is_open()) {
        std::cerr << "Error opening file." << std::endl;
        return 1;
    }
    // Read data from the file
    double F1ggm, F3ggm, F6ggm, Fadjm;
    while (inputFile2 >> Y >> pTm >> F1qgm >> F2qgm >> F1ggm >> F3ggm >> F6ggm >> Fadjm) {
        YValues2.push_back(Y);
        pTValues2.push_back(pTm);
    ...
    const int ndim2 = 8; /* number of dimensions */
    ...
    filename << "dSigma_dDeltaPhi_dihadron_DIS_REP_Trigger" << startid << "_" << endid << "_" <<  stage 
   ...
   //Trigger[0] = 1.0;
    /* Define the integration parameters */
    const int ndim = 10; /* number of dimensions */
