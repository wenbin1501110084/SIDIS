...
//    inter_F2qgVt = exp(inter_F2qgVt);
//    inter_F1ggVt = exp(inter_F1ggVt);
//    inter_F3ggVt = exp(inter_F3ggVt);
...
//    inter_F1qgVt2 = exp(inter_F1qgVt2);
//    inter_F2qgVt2 = exp(inter_F2qgVt2);
//    inter_F1ggVt2 = exp(inter_F1ggVt2);
...
//    inter_FadjVt = exp(inter_FadjVt);
    ...
    /*
    double DFuud1 = 3.*FF->xfxQ2(2, z2, mub2)/z2 * pdf->xfxQ2(2,xq, mub2) + 3.*FF->xfxQ2(1, z2, mub2)/z2 * pdf->xfxQ2(1,xq, mub2);
    double DFuudDg1 = DFuud1 * FF->xfxQ2(21, z1, mub2)/z1;
    double DFuud2 = 3.*FF->xfxQ2(2, z1, mub2)/z1 * pdf->xfxQ2(2,xq, mub2) + 3.*FF->xfxQ2(1, z1, mub2)/z1 * pdf->xfxQ2(1,xq, mub2);
    double DFuudDg2 = DFuud2 * FF->xfxQ2(21, z2, mub2)/z2;
    double TwofgDqDq = 2.*pdf->xfxQ2(21,xq, mub2) * (
                       2.*FF->xfxQ2(2, z1, mub2)/z1 * FF->xfxQ2(2, z2, mub2)/z2 + 2.*FF->xfxQ2(1, z1, mub2)/z1 * FF->xfxQ2(1, z2, mub2)/z2 +
                       2.*FF->xfxQ2(3, z1, mub2)/z1 * FF->xfxQ2(3, z2, mub2)/z2
                       );
    double DFggg = 2.*FF->xfxQ2(21, z1, mub2)/z1 * pdf->xfxQ2(21,xq, mub2) * FF->xfxQ2(21, z2, mub2)/z2;
    */
    ...
    double DFggg = 2.*gl1/z1 * pdf->xfxQ2(21,xq, mub2) * gl2/z2;
    ...
    f[3] = (dsigmaqgqg_dDeltaphi + dsigmagggg_dDeltaphi + dsigmaggqq_dDeltaphi) * qT * total_volume;
    return 0;
}
...
//    inter_FadjVt = exp(inter_FadjVt);
    ...
    /*
    double sumq =  3.*pdf->xfxQ2(2,xq, Q2) * FF->xfxQ2(2, z1, Q2)/z1 + 3.*pdf->xfxQ2(1,xq, Q2) * FF->xfxQ2(1, z1, Q2)/z1;
    double sumg =  2.*pdf->xfxQ2(21,xq, Q2) * FF->xfxQ2(21, z1, Q2)/z1;
    */
            int is=1, ih=1, ic=0, io=1;
    double u1, ub1, d1, db1, s1, sb1, c1, bb1, gl1;
...
int main(int argc, char* argv[]) 
{
    ...
    // Open the input file
    std::ifstream inputFile("Paul_table/Regularged_FT_Nucleus_MV_Paul");
    ...
    LHAPDF::initPDFSet("MSTW2008nlo68cl");
    pdf = LHAPDF::mkPDF("MSTW2008nlo68cl", 0);
    const LHAPDF::PDFSet set("MSTW2008nlo68cl"); // arxiv: 2101.04664
    LHAPDF::initPDFSet("JAM19FF_pion_nlo");
    FF = LHAPDF::mkPDF("JAM19FF_pion_nlo", 0);  // Use the appropriate member index if there are multiple sets
    ...
    for (int itheta=1+ startid; itheta<endid +1; itheta++) {
        ...
        realA << params.Delta_phi << "  ";
                ...
                llVegas(ndim, ncomp, zh_kp2_b_integrated, &params, nvec, epsrel, epsabs, flags, seed, mineval, maxeval, nstart, 
                        nincrease, nbatch, gridno, NULL, NULL, &neval, &fail, integral, error, prob);
                realA << integral[0] <<  "  " << integral[1] <<  "  " << integral[2] <<  "  " <<  "  " << Trigger[0];
                cout << params.Delta_phi << "  " << (integral[0] + integral[1] + integral[2] )/Trigger[0] <<  "  " << integral[3] / (integral[0] + integral[1] + integral[2]) << endl;
        realA << endl;
    }
    }   
    realA.close();
