...
const double Rmin_Pb = 6.62;//fm
const double Rmin_Au = 6.38; //fm
...
double Sud_gammagqqbar(double PT, double bmag, double Subcoff) {
    ...
    double Sud = Sudp1  + Subcoff * Sudnp;
    //if (Sud <0.) Sud = 0.;
    return Sud;
}

double Wgammag2qqbar(double bpmag, double z1, double z2, double xgamma, double PT, double Subcoff) {
    ...
    double xsi = 2.*Rmin_Pb * Mproton * xgamma / HBARC;
    double fxgamma = 2.*ZPb * ZPb *alpha_em /M_PI / xgamma * ( xsi *  boost::math::cyl_bessel_k(0,xsi) *  boost::math::cyl_bessel_k(1,xsi) -
           xsi*xsi/2. * ( boost::math::cyl_bessel_k(1,xsi) *  boost::math::cyl_bessel_k(1,xsi) -  boost::math::cyl_bessel_k(0,xsi) * boost::math::cyl_bessel_k(0,xsi)) );
    double bWbt = 2.*M_PI * bpmag * Dqxqx1 * xgamma * fxgamma * exp(-1.*Sud_gammagqqbar(PT, bpmag, Subcoff));
    //if (bWbt < 0.) bWbt = 0.;
    return bWbt;
}
...
int zh_kp2_b_integrated(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    double xgamma = (kT1 * exp(y1)  + kT2 * exp(y2))/rootsnn;
    //cout << xg << endl;
    if (xg > 1. || xgamma >1.) {
        f[0] = 0.0;
        f[1] = 0.0;
    ...
    double Hgammag2qqbar = alpha_em * zm1 *(1.-zm1) * (zm1*zm1 +(1.-zm1) *(1.-zm1))/PT2/PT2;
    ...
    std::vector<double> xValues, FWWt;
    xValues.clear(); FWWt.clear();
    ...
    double Wgammag2qqbarK = ogata0.fbt(std::bind(Wgammag2qqbar, std::placeholders::_1, z1, z2, xgamma, PT, Subcoff), kminuskp_mag);
    double prefactor_DIS = p1mag * p2mag /z1/z1/z2/z2 /4./M_PI/M_PI * kpmag * Wgammag2qqbarK * total_volume;
    double dsigmaqgqg_dDeltaphi =  prefactor_DIS * inter_FWWt * Hgammag2qqbar;
...
int main(int argc, char* argv[]) 
{
    ...
    // Open the input file
    std::ifstream inputFile("Paul_table/Regularged_FWW_Large_nucleus_MV_Paul");
    ...
    const cubareal epsrel = 1e-50; /* relative error tolerance */
    const cubareal epsabs = 1e-50; /* absolute error tolerance */
    ...
    filename << "dSigma_dDeltaPhi_dihadron_UPC_" << startid << "_" << endid << "_" <<  stage 
