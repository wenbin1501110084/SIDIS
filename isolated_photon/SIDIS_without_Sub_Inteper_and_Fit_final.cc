...
const long long int sample_points = 100000000;
...
int zh_kp2_b_integrated(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    double qdotk = (1.-z) / z * kTg * kTg + z / (1.-z) * PhT * PhT  / zh/zh - 2. * PhT  / zh * kTg * cos(helper->Delta_phi);
    ...
    if (xg >= 0.01) { // Using the Match
        double ap = pdf->xfxQ2(21, xg, 2.5) / (pdf->xfxQ2(21, 0.01, 2.5));
        ...
        if (dipole_model == 1) {
            /*
            std::vector<double> xValues(KValues.begin(), KValues.begin() + Klength);
            auto subsetBegin = NrValues.begin();
            auto subsetEnd = NrValues.begin() + Klength;
            std::vector<double> yValues(subsetBegin, subsetEnd);
            */
            ...
            for (int inn= 0; inn <  Klength; inn++) {
               ...
               // cout << "dddddddddddddd        " << xValues[inn] << "  " << yValues[inn] << endl;
            }
            //for (int iok=0; iok<1000; iok++) {
            //    cout << iok << "  fgfs  " << xValues[iok] << endl;
           // }
            if (kpmag < 2.) {
        ...
        if (dipole_model == 1) {
            ...
            if ( kpmag < 2.) {
                /*
                auto subsetBeginr = KValues.begin() + y_index * Klength;
                auto subsetEndr = KValues.begin() + y_index * Klength + Klength;
                std::vector<double> xValues(subsetBeginr, subsetEndr);
            
                auto subsetBegin = NrValues.begin() + y_index * Klength;
                auto subsetEnd = NrValues.begin() + y_index * Klength + Klength;
                std::vector<double> yValues(subsetBegin, subsetEnd);
                */
                //std::vector<double> xValues, yValues;
                //xValues.clear(); yValues.clear();
                ...
                /*
                auto subsetBeginr2 = KValues.begin() + y_index * Klength;
                auto subsetEndr2 = KValues.begin() + y_index * Klength + Klength;
                std::vector<double> xValues2(subsetBeginr2, subsetEndr2);
            
                auto subsetBegin2 = NrValues.begin() + y_index * Klength;
                auto subsetEnd2 = NrValues.begin() + y_index * Klength + Klength;
                std::vector<double> yValues2(subsetBegin2, subsetEnd2);
                */
