...
int S_Sub(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    double as = 4.*M_PI /(0.75 * log(mu2/Lambda2) + 100.);//50: regulator
    //double as = 0.118;
...
int zh_kp2_b_integrated(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    if (dsigma_dDeltaphi < 0.0) cout << "ddddddddd " << dsigma_dDeltaphi << "  " <<  Wk << "  " << sigmahat << "   " << endl;
...
int main(int argc, char* argv[]) {
    ...
    sprintf(output_filename,"dSigma_dDeltaPhi_with_Sub");
