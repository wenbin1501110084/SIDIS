...
#for ievent in range(11):
    ...
    #print(np.max(allxg))
    levels = np.linspace(0.0, np.max(allxg), 100)
    levels2 = np.linspace(0.0, np.max(allxg), 5)
    ...
    pl.title('<$k_{T}>$ in p-p 200 GeV, $|\eta_{\gamma}|<0.35, |\eta_{h}|<0.35$', fontsize=14)# give plot a title
    pl.savefig("kT_RHIC.png".format(ievent))
    ...
    levels = np.linspace(0.0, np.max(allxg), 100)
    levels2 = np.linspace(0.0, np.max(allxg), 10)
    #levels = np.linspace(np.min(T00),np.max(T00),50)
    cs = pl.contourf(X, Y, allxg, levels=levels,colorscale='Electric', fill=False,cmap=pl.cm.jet)#cmap="Blues")
    cbar = pl.colorbar(cs)
    ...
    pl.xticks(fontsize=14)
    pl.yticks(fontsize=15)
    pl.title('<$k_{T}>$ in p-p 5.02 TeV, $|\eta_{\gamma}|<0.67, |\eta_{h}|<0.8$', fontsize=14)# give plot a title
    pl.savefig("kT_LHC.png".format(ievent))
    ...
    pl.xticks(fontsize=14)
    pl.yticks(fontsize=15)
    pl.title('<$k_{T}>$ in p-p 5.02 TeV, $1.5<\eta_{\gamma}, \eta_{h}<4.0$', fontsize=14)# give plot a title
    pl.savefig("kT_LHCb.png".format(ievent))
