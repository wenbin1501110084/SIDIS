...
aa=np.loadtxt("Large_nucleus_MvGamma_F_table_K_A_all")
...
for iy in range(80):
    ...
    cc = np.loadtxt("/home/wenbin/Downloads/Wenbin_working/Work/WSU_BNL_work/Dijet/photon_hadron/FTtable/FT_table_K_pMVgamma_Paul_large_nucleus_{}_{}.txt".format(int(iy), int(iy+1)))
    cc[0,1] = 0.00001
    x1 = cc[2,1]
    x2 = cc[3,1]
    ...
    if (iy < 30):
        ...
        params, covariance = curve_fit(model_func2c, np.log(rr), np.log(FF))#, p0=initial_guesses)
        predictions = model_func2c(np.log(pp), params[0], params[1], params[2])
        aa[int(iy*700+init2):int(iy*700+700),2] = np.exp(predictions)
        ...
        params, covariance = curve_fit(model_func2c, np.log(rr), np.log(FF))#, p0=initial_guesses)
        predictions = model_func2c(np.log(pp), params[0], params[1], params[2])
        aa[int(iy*700+init2):int(iy*700+700),3] = np.exp(predictions)
    ...
    pl.ylim(1e-6,1)
    print(iy*0.2)
    pl.title('y = {}, MV gamma Paul, large nucleus'.format(iy*0.2), fontsize=15)# give plot a title
    pl.xlabel('$k_{T}$ [GeV]', fontsize=15)# make axis labels
    pl.ylabel('$\\alpha_s F/S_{\perp}$', fontsize=12)
    pl.xticks(fontsize=10)
    pl.yticks(fontsize=12)
    #pl.show()# show the plot on the screen
    pl.savefig('PNG_Large/F_{}.png'.format(iy), dpi=120)

np.savetxt("Regularged_FT_large_Nuleus_MVgamma_Paul",
