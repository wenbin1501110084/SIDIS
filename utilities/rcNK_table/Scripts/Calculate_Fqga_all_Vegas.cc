...
int zh_kp2_b_integrated(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    */
    ...
    double alphas_Fadj = -2./3. * ST / M_PI /M_PI * bpmag/ 2./M_PI * 2.25 * 1.25 * pow(S0, 0.25) *  DrN0*DrN0 + 2.25 * pow(S0, 1.25) * DDrN0;
...
{
    // Open the input file
    std::ifstream inputFile("Paul_table/rcBK-MVe_large_nucleus.txt");
    ...
    // Open the input file
    std::ifstream inputFileS("Paul_table/Delta_r_N_paul_rcBK-MVe_large_nucleus.txt");
