...
const long long int sample_points = 1000;
...
double Wb(double b, double zh, double xp, double Q2, double xg){
    ...
    double Lambda2 = 0.0542;//GeV^2 
    double A = 0.897;
    double B = -1.341;// A and B are values of the Sudakov only
...
int zh_kp2_b_integrated(const int *ndim, const cubareal *x, const int *ncomp, cubareal *f, void *userdata){
    ...
    double qdotk = 0.5 * kTg * kTg / z/(1.-z); // back-to-back
    ...
    double  dsigma_dDeltaphi = 0.025330295910584444 / zh/zh * Wk * sigmahat * kTg * PhT; // Jacobe
...
int main(int argc, char* argv[]) {
    ...
    filename << "dSigma_dDeltaPhi_Sud_only_FBT_" << startid << "_" << endid << "_" <<  stage <<".txt";
