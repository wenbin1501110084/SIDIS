...
for iq in range(len(Q2)):
    ...
    B = -alphas/math.pi *1.5*CF - alphas * 2* CA * 0.75 /2/math.pi
...
beta0 = (11-2)/12
for iq in range(len(Q2)):
    mu2 = np.arange(mub2, Q2[iq], 0.001)
    alphas = model_func2d(mu2, paramsaa[0], paramsaa[1])
    stem = 1/mu2 * (2*(CF+CA)*alphas/math.pi/2 * np.log(Q2[iq]/mu2) - (1.5 * CF + CA *beta0) * alphas/math.pi/2)
    integral_coherent = simps(stem, mu2)
    SSud.append(integral_coherent)
SSud = np.array(SSud)
pl.figure(100402)
...
predictions = model_func2c(Q2, 1.3914805,  -0.69373394)
pl.plot(Q2, (predictions),'k--',label='CGC, W.O. Sub')

#pl.xlim(0.31,2*pi-0.31)
#pl.ylim(0,0.14)
pl.title('pp 200 GeV, 5 < $k_{\gamma \perp} $ < 7 GeV, 0.5<$P_{h \perp}$ < 1 GeV', fontsize=15)# give plot a title
pl.xlabel('Q2', fontsize=15)# make axis labels
pl.ylabel('Q2', fontsize=12)
pl.xticks(fontsize=10)
pl.yticks(fontsize=12)
#pl.show()# show the plot on the screen
pl.savefig('SSud_Q2_qgqg.png', dpi=120)
...
for iq in range(len(Q2)):
    mu2 = np.arange(mub2, Q2[iq], 0.001)
    alphas = model_func2d(mu2, paramsaa[0], paramsaa[1])
    stem = 1/mu2 * (4*CA*alphas/math.pi/2 * np.log(Q2[iq]/mu2) - 3 *  CA *beta0 * alphas/math.pi)
    integral_coherent = simps(stem, mu2)
    SSud.append(integral_coherent)
SSud = np.array(SSud)
pl.figure(100322402)
...
predictions = model_func2c(Q2,  1.90103746, -2.15857456)
pl.plot(Q2, (predictions),'k--',label='CGC, W.O. Sub')
...
pl.yticks(fontsize=12)
#pl.show()# show the plot on the screen
pl.savefig('SSud_Q2_gggg.png', dpi=120)
